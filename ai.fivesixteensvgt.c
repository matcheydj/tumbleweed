...
void start_timer() {
    ...
    printf("<animate id=\"timer\" attributeName=\"content\" values=\"5;4;3;2;1\" keyTimes=\"0;0.2;0.4;0.6;0.8;1\" dur=\"5s\" fill=\"freeze\"/>\n"); // Animate the text to change the timer value
    printf("<set attributeName=\"display\" to=\"none\" begin=\"timer.end+1s\"/>\n"); // Hide the text after the timer ends and waits for 1 second
    printf("</svg>\n"); // Print the closing SVG tag
    printf("<script>\n"); // Print the script tag
    printf("setTimeout(function() { display_choices(); }, 6000);\n"); // Set a timeout to display the choices after 6 seconds
...
int main() {
    ...
    printf("%s\n", display_choices); // Print the C code for the display_choices function as JavaScript
    printf("</script>\n"); // Print the closing script tag
